...
::: {.callout-important}
Es wurden bisher _n_=`r (ncol(darts)-2)/2` junge und _n_=`r (ncol(darts_old)-2)/2` alte Proband*innen erhoben. 
:::
...
# Daten (10x2er Blöcke, gesamt 20 Blöcke)
...
kable(anova_table_mean_2, caption = "Within-Subject Effects (partielles η²)")
...
anova_table_median_2 <- anova_result_mean_2$ANOVA %>%
  mutate(
    etasq_p = SSn / (SSn + SSd)   # partielles eta²
  ) %>%
  select(Effect, DFn, DFd, SSn, SSd, F, p, etasq_p) %>%  # nur relevante Spalten
...
# Daten (5x4er Blöcke, gesamt 20 Blöcke)
...
::: {.callout-important}
Hier habe ich `r_n_=7` Proband*innen ausgewertet.
:::
